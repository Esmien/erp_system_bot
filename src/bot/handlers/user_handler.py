...
from bot.keyboards.reply_keyboard import BaseActions, get_main_keyboard
...
async def get_my_info(message: types.Message, state: FSMContext, user_client: ApiUserClient):
    ...
    msg = await message.answer(text=response_text, reply_markup=get_main_keyboard(is_auth=True, role=my_info.role.name))
